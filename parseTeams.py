...
data_other = pd.read_csv("./Data/SchoolStats/school_2010.csv")
...
differenceSet = set(kaggleTeams).difference(otherTeams)

# Add functionality to delete extra rows in kaggleData since it has more schools
for id, name in data_kaggle["TeamName"].iteritems():
    for diff in differenceSet:
        if(name == diff):
            print(name)
            data_kaggle.drop(index=id)

data_kaggle.drop(differenceSet)
